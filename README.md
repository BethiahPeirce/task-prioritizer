...
grade, the app uses Claude to rank which should be completed first.

## Why I built this
I built this tool so I could expand my knowledge on using LLM API in my programming.
...
prioritization tradeoffs, an important skill that product and project managers
need to control a backlog of tasks. I am now able to compare my own decision 
making processes with Claude's and tests which are more realistic and achievable.
...
3. After you hit the Prioritize button, the data is sent to Claude, who reasons through the tradeoffs and returns a ranked list of what to complete first along
with explanations.
...
4.  Run the app (python3 -m streamlit run app.py)

## What I Learned
Similarly to what I do when reasoning my task completion in real life, I found
...
Sometimes this resulted in an assignment being underweighted if the due date was
far enough away, so I still feel that a PM's judgement about what would be best for
a business would still be needed in these kinds of situations.
It also prioritized completing tasks that took less effort first, as Claude also
